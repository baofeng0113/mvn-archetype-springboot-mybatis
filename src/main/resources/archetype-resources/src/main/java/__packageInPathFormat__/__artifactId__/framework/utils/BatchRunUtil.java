package ${package}.${artifactId}.framework.utils;
...
public class BatchRunUtil {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(BatchRunUtil.class);

    private static final ForkJoinPool forkJoinPool = new ForkJoinPool(
            Runtime.getRuntime().availableProcessors() * 2);

    public int batchRunMethod(@NonNull List<Runnable> tasks) {
        Future<Integer> result = forkJoinPool.submit(new CustomTask(tasks));
        try {
            int executedTaskAmount = result.get();
            LOGGER.info("executed task amount: {}", executedTaskAmount);
